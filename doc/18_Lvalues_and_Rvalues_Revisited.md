...
`binexpr()`には明示的にツリーを右辺値とセットする数行のコードが散らばっています。これらは葉ノードに入ると実行されます。例えば`b=a`にある識別子aは右辺値としてマークされる必要がありますが、それをするためのwhileループのボディへ入ることはありません。

## ツリーの書き出し

パーサの変更は一旦おいておきます。今はいくつか右辺値としてマークされているノードもあれば、全くマークされていないノードもあります。この時点で生成されるASTツリーの可視化に問題があることに気が付きました。`tree.c`に`dumpAST()`という関数を記述し、それぞれのASTツリーを標準出力に書き出すようにしました。洗練されていません。これでコンパイラに内部フラグ`O_dumpAST`をセットする`-T`コマンドライン引数が用意できました。`decl.c`にある`global_declarations()`コードは次のようになっています。
...
34はcharのサイズのリテラルとしては十分に小さいですが`b`の型に合うように拡張されます。`A_IDENT b`は"右辺値"とは書かれていないので左辺値です。34という値は左辺値`b`に格納されています。この値は左辺値`a`へ格納されます。
...
"rval `b`"が確認できるので、`b`の値がレジスタに読み込まれ、`b+34`の式の結果が左辺値`a`に格納されます。
...
今回はここに書いた解決策にたどり着くまでに2つ3つの設計案を取り、実際に試し、行き詰まったり後戻りしたように思います。SubCにおいてNilsがある時点で処理されるASTツリーのノードの"左辺値"らしさを保持するため、単一の"左辺値"構造体を渡すことは知っています。ですが彼のツリーは1つの式だけを保持しており、こちらのコンパイラのASTツリーは1つの関数全体に相当するノードを保持しています。もしあなたが3種類のコンパイラを調査したら、おそらく3つの解決策を見つけることになると思います。

次に取り掛かるものは多くあります。Cにはオペレータが大量にあり、これらはコンパイラに追加するのが比較的容易です。A_SCALEがあるので構造体も試せます。ローカル変数はまだないので、いずれやらなければなりません。そして関数を一般化して複数の引数を取れるようにし、それらにアクセスできるようにする必要があります。

次回は配列に取り掛かります。これは間接参照、左辺値、右辺値の組み合わせで、さらに配列要素のサイズによる配列インデックスのスケーリングとなるでしょう。必要な意味解釈のコンポーネントは揃っていますが、トークン、パース、インデックス機能を追加する必要があります。今回のような面白いトピックとなるでしょう。
